"""
Simple Generator for AUTOSAR XML
(supports only one CAN network)
"""
import struct
...
class Generator():
    ...
    (supports only one CAN network)
    """
    def __init__(self, db_name="CAN_DB", cluster="CAN_Cluster", channel_name="CAN_Channel"):
    ...
    def init_can(self):
        ...
        self.root = etree.Element('AUTOSAR',
        ...
        cluster_var = AddElem(ap_cluster, self.cluster_name, "CAN-CLUSTER", "CAN-CLUSTER-VARIANTS")
    ...
    def add_ecu(self, ecu_name):
        comm_connectors = self.root.find(".//COMM-CONNECTORS")
        add_dict(comm_connectors, {"COMMUNICATION-CONNECTOR-REF-CONDITIONAL": {
            "COMMUNICATION-CONNECTOR-REF": {'@DEST':"CAN-COMMUNICATION-CONNECTOR", '#text': f"/{self.gen_name}/ECU_INSTANCES/{ecu_name}/Connector_{ecu_name}"}}}
        )
        ecu_instances = self.root.find(".//AR-PACKAGE[SHORT-NAME='ECU_INSTANCES']/ELEMENTS") 
        ...
        etree.SubElement(con, "COMM-CONTROLLER-REF", DEST="CAN-COMMUNICATION-CONTROLLER").text = \
            f"/{self.gen_name}/ECU_INSTANCES/{ecu_name}/Controller_{ecu_name}"
        
        ports = etree.SubElement(con, "ECU-COMM-PORT-INSTANCES")


    def add_frame(self, frm_name, can_id, dlc, ecu_tx, ecu_rx, pdu):
        frm_elems = self.root.find(".//AR-PACKAGE[SHORT-NAME='FRAME']/ELEMENTS") 
        add_dict(frm_elems, {"CAN-FRAME":{
            ...
            "PDU-TO-FRAME-MAPPINGS": {"PDU-TO-FRAME-MAPPING":{
                ...
                "PDU-REF": {"@DEST":"I-SIGNAL-I-PDU", "#text": f"/{self.gen_name}/PDUS/{pdu}"},
                "START-POSITION": "0",
            }}
        }})

        pdu_elems = self.root.find(".//AR-PACKAGE[SHORT-NAME='PDUS']/ELEMENTS") 
        ...
        pdu_triggerings = self.root.find(".//CAN-PHYSICAL-CHANNEL/PDU-TRIGGERINGS")    
        ...
        etree.SubElement(port_refs, "I-PDU-PORT-REF", DEST="I-PDU-PORT").text = \
            f"/{self.gen_name}/ECU_INSTANCES/{ecu_tx}/Connector_{ecu_tx}/PduPort_{ecu_tx}_TX_{pdu}"
        etree.SubElement(port_refs, "I-PDU-PORT-REF", DEST="I-PDU-PORT").text = \
            f"/{self.gen_name}/ECU_INSTANCES/{ecu_rx}/Connector_{ecu_rx}/PduPort_{ecu_rx}_RX_{pdu}"    
        etree.SubElement(trigger, "I-PDU-REF", DEST="I-SIGNAL-I-PDU").text = f"/{self.gen_name}/PDUS/{pdu}"
        sig_trigs = etree.SubElement(trigger, "I-SIGNAL-TRIGGERINGS") 

        ports = self.root.find(f".//ECU-INSTANCE[SHORT-NAME='{ecu_tx}']/CONNECTORS/CAN-COMMUNICATION-CONNECTOR/ECU-COMM-PORT-INSTANCES")
        ...
        ports = self.root.find(f".//ECU-INSTANCE[SHORT-NAME='{ecu_rx}']/CONNECTORS/CAN-COMMUNICATION-CONNECTOR/ECU-COMM-PORT-INSTANCES")
        ...
        triggerings = self.root.find(".//FRAME-TRIGGERINGS")
        f = {
            'CAN-FRAME-TRIGGERING':
            {
                ...
                [
                    {'@DEST':"FRAME-PORT", '#text':f"/{self.gen_name}/ECU_INSTANCES/{ecu_tx}/Connector_{ecu_tx}/FramePort_{ecu_tx}_TX_{frm_name}" },
                    {"@DEST":"FRAME-PORT", '#text':f"/{self.gen_name}/ECU_INSTANCES/{ecu_rx}/Connector_{ecu_rx}/FramePort_{ecu_rx}_RX_{frm_name}" }
                ]},
                "FRAME-REF":{'@DEST':"CAN-FRAME", '#text':f"/{self.gen_name}/FRAME/{frm_name}"},
                "PDU-TRIGGERINGS":{
                    "PDU-TRIGGERING-REF-CONDITIONAL":{
                        "PDU-TRIGGERING-REF": {'@DEST':"PDU-TRIGGERING", '#text':f"/{self.gen_name}/{self.db_name}/{self.cluster_name}/{self.channel_name}/PduTriggering_{pdu}" }
                    }
                },            
    ...
    def add_signal(self, sig_name, len, sig_type, compu_meth='FACTOR_1'):
        sig_elems = self.root.find(".//AR-PACKAGE[SHORT-NAME='I_SIGNALS']/ELEMENTS") 
        ...
        etree.SubElement(cond, "BASE-TYPE-REF", DEST="SW-BASE-TYPE").text = \
            f"/{self.gen_name}/BASE_TYPES/{sig_type}"
        etree.SubElement(sig, "SYSTEM-SIGNAL-REF", DEST="SYSTEM-SIGNAL").text = \
            f"/{self.gen_name}/SYSTEM_SIGNALS/{sig_name}"

        sig_triggerings = self.root.find(".//CAN-PHYSICAL-CHANNEL/I-SIGNAL-TRIGGERINGS")
        ...
        etree.SubElement(trigger, "I-SIGNAL-REF", DEST="I-SIGNAL").text = \
                f"/{self.gen_name}/I_SIGNALS/{sig_name}"

        syssig_elems = self.root.find(".//AR-PACKAGE[SHORT-NAME='SYSTEM_SIGNALS']/ELEMENTS")
        add_dict(syssig_elems, {"SYSTEM-SIGNAL":{
            "SHORT-NAME":sig_name, 
            "PHYSICAL-PROPS": {
                "SW-DATA-DEF-PROPS-VARIANTS": {
                    "SW-DATA-DEF-PROPS-CONDITIONAL": {
                        "COMPU-METHOD-REF": {"@DEST":"COMPU-METHOD", "#text": f"/{self.gen_name}/COMPUMETHODS/{compu_meth}"}
                    }
                }
            }
            }})
    ...
    def add_signal_mapping(self, pdu_name, sig_name, start_bit, ecu_tx, ecu_rx):
        sig_maps = self.root.find(f".//I-SIGNAL-I-PDU[SHORT-NAME='{pdu_name}']/I-SIGNAL-TO-PDU-MAPPINGS")         
        ...
        sig_trigs = self.root.find(f".//PDU-TRIGGERING[SHORT-NAME='PduTriggering_{pdu_name}']/I-SIGNAL-TRIGGERINGS")
        ...
        etree.SubElement(refs, "I-SIGNAL-TRIGGERING-REF",DEST="I-SIGNAL-TRIGGERING").text = \
            f"/{self.gen_name}/{self.db_name}/{self.cluster_name}/{self.channel_name}/SignalTriggering_{sig_name}"


        refs2 = self.root.find(f".//I-SIGNAL-TRIGGERING[SHORT-NAME='SignalTriggering_{sig_name}']/I-SIGNAL-PORT-REFS")
        etree.SubElement(refs2, "I-SIGNAL-PORT-REF", DEST="I-SIGNAL-PORT").text = \
            f"/{self.gen_name}/ECU_INSTANCES/{ecu_tx}/Connector_{ecu_tx}/SignalPort_{ecu_tx}_TX_{sig_name}" 
        etree.SubElement(refs2, "I-SIGNAL-PORT-REF", DEST="I-SIGNAL-PORT").text = \
            f"/{self.gen_name}/ECU_INSTANCES/{ecu_rx}/Connector_{ecu_rx}/SignalPort_{ecu_rx}_RX_{sig_name}" 
    ...
    def add_pydb_frame(self, ecu_name, gw_name, can_id, frm_name, direction, formats, signals):
        ...
        pdu = frm_name
        self.add_frame(frm_name, can_id, dlc, ecu_tx, ecu_rx, pdu)

        startbit = 0
        for n in range(len(signals)):
            sig = signals[n]
            sig_name = sig.replace('.', '_').replace('/', '_')
            sig_type = self.TYPES[formats[n]]
            sig_len = struct.calcsize(formats[n]) * 8
            self.add_signal(sig_name, sig_len, sig_type)

            self.add_signal_mapping(pdu, sig_name, startbit, ecu_tx, ecu_rx)
            startbit += sig_len
